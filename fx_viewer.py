...
# Custom CSS - Enhanced for better reliability
st.markdown("""
<style>
    /* Prevent external resource loading issues */
    * {
...
# ============================================================================
# HELPER FUNCTIONS FOR BINOMIAL MODEL
# ============================================================================

def simple_norm_cdf(x, mean=0, std=1):
    """Simplified normal CDF approximation"""
    z = (x - mean) / std
    # Simple approximation for normal CDF
    if z >= 0:
        return 0.5 * (1 + math.sqrt(1 - math.exp(-2 * z * z / math.pi)))
    else:
        return 1 - simple_norm_cdf(-z)
    ...
    try:
        ...
        try:
            if historical_data['success'] and len(historical_data['rates']) >= 20:
                ...
                # Use simplified normal CDF instead of scipy
                p_up_empirical = 1 - simple_norm_cdf(current_spot, mean_20_days, std_20_days)
        ...
        try:
            ...
            for day in range(6):
                if day == 0:
                    ...
                    for j in range(day + 1):  # j może być od 0 do day
                        ...
                        node_prob = calculate_binomial_coefficient(day, j) * (p ** j) * ((1 - p) ** (day - j))
            ...
            with col2:
                prob = calculate_binomial_coefficient(final_day, final_j) * (p ** final_j) * ((1 - p) ** (final_day - final_j))
            ...
            # DRZEWO DWUMIANOWE WIZUALIZACJA - Safer version
            st.subheader("🌳 Drzewo Dwumianowe")
            
            try:
                fig = go.Figure()
                
                # Plot nodes with safer approach
                for day in range(6):
                    for j in range(day + 1):
                        rate = tree[day][j]
                        x = day
                ...
                st.error(f"Błąd wizualizacji: {str(plot_error)}")
            ...
            **Jak znajduję najczęstszą ścieżkę:**
            ...
            but ścieżka przechodząca przez najczęstsze węzły w każdym dniu.
            """)
            ...
            if st.checkbox("Pokaż szczegóły prawdopodobieństw", value=False):
                try:
                    ...
                    for day in range(1, 4):  # Dni 1, 2, 3
                        for j in range(day + 1):
                            ...
                            prob_data.append({
                                ...
                                "Ruchy w górę (j)": j,
                                "Ruchy w dół": day - j,
                                "C(n,j)": calculate_binomial_coefficient(day, j),
                    ...
                    st.error(f"Błąd tabeli: {str(table_error)}")
            
        except Exception as tree_error:
            st.error(f"Błąd budowy drzewa: {str(tree_error)}")
            st.info("Spróbuj użyć innych parametrów lub odśwież stronę.")
    
    except Exception as main_error:
        st.error(f"Główny błąd panelu: {str(main_error)}")
        st.info("Panel chwilowo niedostępny. Spróbuj ponownie.")
        
        # Fallback - show basic info
        st.subheader("📊 Podstawowe Informacje")
        current_forex = get_eur_pln_rate()
        st.metric("Aktualny kurs EUR/PLN", f"{current_forex['rate']:.4f}")
        st.info("Pełny model zostanie załadowany po rozwiązaniu problemów technicznych."):
...
# ============================================================================
# MAIN APP - Enhanced with error handling
# ============================================================================
        ...
        try:
            ...
            with tab1:
                try:
                    ...
                    st.error(f"Błąd panelu dealerskiego: {str(e)}")
            ...
            with tab2:
                try:
                    ...
                    st.error(f"Błąd panelu zabezpieczeń: {str(e)}")
                    st.info("Upewnij się, że panel dealerski został skonfigurowany.")
            
            with tab3:
                try:
                    create_binomial_model_panel()
                except Exception as e:
                    st.error(f"Błąd modelu dwumianowego: {str(e)}")
                    st.info("Model chwilowo niedostępny. Dane będą dostępne po naprawie.")
                    
        except Exception as tab_error:
            st.error(f"Błąd interfejsu: {str(tab_error)}")
            st.markdown("## 🔧 Tryb Awaryjny")
            st.info("Aplikacja działa w trybie ograniczonym. Niektóre funkcje mogą być niedostępne.")
            
            # Basic functionality
            st.subheader("📊 Podstawowe Dane")
            try:
                current_forex = get_eur_pln_rate()
                st.metric("EUR/PLN", f"{current_forex['rate']:.4f}")
                st.caption(f"Źródło: {current_forex['source']}")
            except:
                st.warning("Dane niedostępne")
    
    except Exception as main_error:
        st.error(f"Krytyczny błąd aplikacji: {str(main_error)}")
        st.markdown("## ⚠️ Aplikacja Niedostępna")
        st.info("Wystąpił problem z ładowaniem aplikacji. Skontaktuj się z administratorem.")
        
        # Emergency fallback
        st.markdown("### 🆘 Tryb Awaryjny")
        st.write("EUR/PLN: ~4.25 (przybliżony kurs)")
        st.write("Status: Aplikacja w trybie offline")

# Enhanced error handling for the app launch
if __name__ == "__main__":
    try:
        main()
    except Exception as startup_error:
        st.error(f"Błąd uruchomienia: {str(startup_error)}")
        st.markdown("## 🔄 Spróbuj Ponownie")
        st.info("Odśwież stronę lub sprawdź połączenie internetowe.")
