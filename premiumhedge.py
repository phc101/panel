...
def main():
    ...
    if fx_file and dom_yield_file and for_yield_file:
        ...
        # Calculate Yearly Returns and Number of Trades
        result_df["Year"] = pd.to_datetime(result_df["Entry Date"]).dt.year
        yearly_returns = result_df.groupby("Year")["Revenue %"].sum()
        yearly_trades = result_df.groupby("Year").size()
        
        # Plot Yearly Returns Bar Chart with Number of Trades on Top
        fig, ax = plt.subplots()
        bars = yearly_returns.plot(kind='bar', ax=ax, color='blue')
        ax.set_title(f"Yearly Returns ({strategy})")
        ax.set_xlabel("Year")
        ax.set_ylabel("Total Revenue %")
        
        for bar, trades in zip(bars.patches, yearly_trades):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{trades}', ha='center', va='bottom', fontsize=10, color='black')
